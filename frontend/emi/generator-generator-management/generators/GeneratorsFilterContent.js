...
function TodoSidebarContent(props) {
    ...
    const { filters: { active: activeChecked } } = useSelector(({ GeneratorManagement }) => GeneratorManagement.generators);
    const T = new MDText(i18n.get(user.locale));


    function handleActiveChange(evt) {
        if (activeChecked === null) {
            dispatch(Actions.setGeneratorsFilterActive(true));
        } else if (activeChecked) {
            dispatch(Actions.setGeneratorsFilterActive(false));
        } else {
            dispatch(Actions.setGeneratorsFilterActive(null));
        }
    }
                ...
                <div className="p-24">
                    <FormGroup row>
                        <FormControlLabel
                            ...
                            label={T.translate("generators.filters.active")}
                        />
                    </FormGroup>
                </div>
